<?php
	...
	function _default()
	{
		...
		$pageSize = 15;
		$totalRecords = $adNews->getTotalbyAdNews();
		$totalPages = intval($totalRecords / $pageSize);
		$totalPages += ($totalRecords % $pageSize == 0 ? 0 : 1);
		if($pageNum > $totalPages) $pageNum = $totalPages;
		if($totalRecords>0)
			$tpl->assign('adNews', $adNews->getbyAdNews($pageNum,$pageSize,0));
		
		$tpl->assign('paggingbar', genPaggingbar('adNews.php',$pageNum,$totalPages));
		$tpl->display("showAdNews.html");
	}
	...
	function _new() {
		...
		$tpl->assign("siteTitle","后台管理系统");		
		$tpl->assign("adNews",$adNews->getAdNewsbyId($id));
		$tpl->display("editAdNews.html");
	}
	...
	function _save() {
		...
		if ($id == 0) {
			$cc = $adNews->insertAdNews($title,$newimg,$contenturl,$checked);
		} else {
			if($newimg=="")
				$imgurl=$hasimg;
			else
			...
			$cc = $adNews->updateAdNews($id,$title,$imgurl,$contenturl,$checked);
		} 
		
		if ($cc)
        	echo "<script>alert('操作成功！');window.location.href='adNews.php';</script>";
    	else
        	echo "<script>alert('操作失败！');window.location.href='adNews.php';</script>";
	}
